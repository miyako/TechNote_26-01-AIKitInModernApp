...
$html:=$html+$body
$html:=$html+"<script>"
$html:=$html+"function addAssistantMessage(content) {"
$html:=$html+"  var row = document.createElement('div');"
$html:=$html+"  row.className = 'message assistant';"
$html:=$html+"  row.innerHTML = '<div class=\"role\">🤖 Assistant</div><div class=\"content\">' + content + '</div>';"
$html:=$html+"  row._raw = content;"
$html:=$html+"  document.body.appendChild(row);"
$html:=$html+"  window.scrollTo(0, document.body.scrollHeight);"
$html:=$html+"}"
$html:=$html+"function appendAssistantMessage(chunk) {"
$html:=$html+"  var msgs = document.querySelectorAll('.message.assistant');"
$html:=$html+"  var last = msgs[msgs.length - 1];"
$html:=$html+"  if (!last) { addAssistantMessage(chunk); return; }"
$html:=$html+"  last._raw = (last._raw || '') + chunk;"
$html:=$html+"  var contentEl = last.querySelector('.content');"
$html:=$html+"  if (contentEl) contentEl.innerHTML = last._raw;"
$html:=$html+"  window.scrollTo(0, document.body.scrollHeight);"
$html:=$html+"}"
$html:=$html+"function addUserMessage(content) {"
$html:=$html+"  var row = document.createElement('div');"
$html:=$html+"  row.className = 'message user';"
$html:=$html+"  row.innerHTML = '<div class=\"role\">👤 You</div><div class=\"content\">' + content + '</div>';"
$html:=$html+"  document.body.appendChild(row);"
$html:=$html+"  window.scrollTo(0, document.body.scrollHeight);"
$html:=$html+"}"
$html:=$html+"window.scrollTo(0, document.body.scrollHeight);"
